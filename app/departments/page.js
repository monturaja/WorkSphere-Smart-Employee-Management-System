...
import { Plus, Building2, PlusCircle } from 'lucide-react';
    ...
    <DashboardLayout>
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 relative overflow-hidden">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-white tracking-tighter">
              {isAdmin ? 'Structural' : isHR ? 'Organizational' : 'Departments'} <span className={theme.accent}>Entities</span>
            </h1>
          </div>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">
            {isAdmin ? 'System-wide Organizational Oversight & Budgetary Control' : isHR ? 'Workforce Allocation & Departmental Strategy' : 'Organize your company structure and budget.'}
          </p>
        </div>
        ...
        >
          ...
          {isAdmin ? 'Initialize Structure' : isHR ? 'Generate Unit' : 'New Department'}
        </button>
      </header>
